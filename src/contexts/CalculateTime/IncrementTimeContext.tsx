...
const api_url = " https://timeapi.io/api/Calculation/custom/increment";
...
const IncrementTimeReducer = (state: TimeZoneState, action: Action) => {
  switch (action.type) {
...
const IncrementTimeContext = createContext(defaultContextValue);
const IncrementTimeProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
      const [
        {
        ...
        dispatch,
      ] = useReducer(IncrementTimeReducer, initialState);
  ...
  return (
    <IncrementTimeContext.Provider value={contextValue}>
      {children}
    </IncrementTimeContext.Provider>
  );
};

export default IncrementTimeProvider;

export const useIncrementTimeContext = () => {
  const context = useContext(IncrementTimeContext);
